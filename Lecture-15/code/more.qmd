...
```{r}
all.equal(
    summary(ols1)$coefficients[2, 3] |> as.numeric(),
    t |> as.numeric()
)
```
...
ci[1] <- add(
    ols1$coefficients[2],
    qt(p = 0.025, df = ols1$df.residual) |>
        multiply_by(se)
)
ci[2] <- add(
    ols1$coefficients[2],
    qt(p = 0.975, df = ols1$df.residual) |>
        multiply_by(se)
)
```
...
```{r}
all.equal(
    summary(ols1)$r.square |> as.numeric(),
    r2_1
)
```
...
```{r}
all.equal(
    summary(ols1)$fstatistic[1] |> as.numeric(),
    f
)
```
...
ggplot(data = ts) +
    ...
    geom_vline(
        xintercept = c(-critical_v, critical_v)
    )
```
