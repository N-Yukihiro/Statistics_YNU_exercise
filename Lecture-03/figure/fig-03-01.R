...
mean_by_group <- function(x) {
    dplyr::summarise(x, mean_x = mean(value), .by = var)
}

median_by_group <- function(x) {
    dplyr::summarize(x, median_x = median(value), .by = var)
}

mode_by_group <- function(x) {
    dplyr::summarise(
        x,
        mode_x = mlv(value, method = "meanshift"),
        .by = var
    )
}
...
# データの作成, グラフの描画
set.seed(2718)
fig_03_01 <- withr::with_preserve_seed(
    tibble(
        x1 = rnorm(n = 100000, mean = 5, sd = 0.1),
